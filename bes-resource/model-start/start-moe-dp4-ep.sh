#!/bin/bash
# 模式② 宽 EP（高吞吐首选）: TP=1, DP=4 + --enable-expert-parallel
# Qwen1.5-MoE-A2.7B  用 0,1,2,3 共 4 卡
# 4 个 DP 副本各占 1 卡跑 attention；60 个专家按 EP 分布在 4 卡；1 个 API server 负载均衡 4 个 engine
#
...
echo "模式② DP4+EP 启动中 (容器 vllm-moe-bench, 端口 9003)"
